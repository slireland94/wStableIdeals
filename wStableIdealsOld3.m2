...
-- gets the smallest lex monomial from the list B that generates v
getSmallestLexMonThatGeneratesMon = method();
getSmallestLexMonThatGeneratesMon (List,RingElement) := RingElement => (B,v) -> (
    S := ring v;
    gs := gens S;
    K := coefficientRing S;
    Slex := K[gs,MonomialOrder=>Lex];
    Bs := sort(for b in B list sub(b,Slex));
    vlex := sub(v,Slex);
    r := #Bs;
    tf := true;
    TT := tableOfTrees(Bs,stopMons=>{vlex});
...
coneWhereShadowsMissEachother List := Cone => B -> (
    ...
    for b in B do (
        ...
        --mb := getSmallestLexMonThatGeneratesMon(delete(b,B),b);
...
coneWhereShadowsGenerateIdeal (List,Ideal) := Cone => (B,I) -> (
    ...
    for v in G do (
        ...
        --mv := getSmallestLexMonThatGeneratesMon(B,v);
